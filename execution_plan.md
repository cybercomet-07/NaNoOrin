# SwarmOS — Production Build Prompt Sequence
> PM + Senior Backend Architect + Senior AI Engineer perspective
> 100% sourced from SwarmOS Project Brief v1.0
...
```
Create the exact SwarmOS folder structure as specified below. Generate every file as an empty stub with correct imports and a TODO comment. Do not write any logic yet.
...
```
Create the three prompt text files for SwarmOS agents based on these exact specifications:

FILE: backend/prompts/supervisor.txt
You are the Head of Digital Workforce (HDW) for SwarmOS. Your objective is to decompose the user's goal into a TaskGraph.
...
You are a senior software developer at SwarmOS.
...
```
Create backend/state.py for SwarmOS. This is the single source of truth for the entire pipeline.
...
```
Create backend/tools/e2b_tools.py for SwarmOS. This wraps the E2B sandbox for the Developer agent.
...
```
Create backend/tools/tavily_tools.py for SwarmOS. This wraps Tavily search for the Researcher agent.
...
```
Create backend/agents/supervisor.py for SwarmOS.
...
```
Create backend/agents/researcher.py for SwarmOS.
...
```
Create backend/agents/persona.py for SwarmOS.
...
```
Create backend/agents/architect.py for SwarmOS.
...
```
Create backend/agents/developer.py for SwarmOS.
...
```
Create backend/agents/critic.py for SwarmOS.
...
```
Create backend/agents/auditor.py for SwarmOS.
...
```
Create backend/graph.py for SwarmOS. This assembles the entire LangGraph directed state graph.
...
```
Create backend/main.py for SwarmOS. This is the production FastAPI server.
...
```
Create backend/tests/test_pipeline.py — a pytest test file that validates the full SwarmOS pipeline.
...
def test_self_healing_wrong_library():
    """
    DEMO TEST: SwarmOS must self-heal from a non-existent library name.
...
```
Create backend/startup_check.py for SwarmOS. Run this before uvicorn starts.
...
def validate_environment():
    ...
    if errors:
        print("\n❌ SwarmOS startup FAILED. Fix these before demo:\n")
        for e in errors:
            print(f"   {e}")
        sys.exit(1)
...
```
Create README.md for SwarmOS with the following sections. Use the project brief as the source of truth.

# SwarmOS — Autonomous Product Lifecycle Engine
